...
- (void) keyDown:(NSEvent *)theEvent
{
	...
	if ([characters length])
	{
		...
		switch (character)
		{
			...
			// Movement
			case 'w':
				cam.pos.x += sin(cam.rot.y)*increment;
				cam.pos.z -= cos(cam.rot.y)*increment;
				cam.pos.y -= sin(cam.rot.x)*increment;
				[self setNeedsDisplay: YES];
				break;
			case 's':
				cam.pos.x -= sin(cam.rot.y)*increment;
			...
			case 'g':
				cam.pos.y += increment*sin(cam.rot.z);
				cam.pos.x -= increment*cos(cam.rot.z);
				[self setNeedsDisplay: YES];
				break;
			case 'h':
				cam.pos.y -= increment*sin(cam.rot.z);
			...
			*/
			...
			case 'i':
				cam.rot.x += 0.0174532925;
				cam.dir.y = sin(cam.rot.x);
				cam.dir.z = -cos(cam.rot.x);
				[self setNeedsDisplay: YES];
				break;
				
			case 'k':
				cam.rot.x -= 0.0174532925;
				NSLog(@"cam.rot.x = %f", cam.rot.x);
				cam.dir.y = sin(cam.rot.x);
				cam.dir.z = -cos(cam.rot.x);
				[self setNeedsDisplay: YES];
				break;
			...
			case 'v':
				cam.rot.z += 0.0174532925;
				cam.dir.y = sin(cam.rot.z);
				cam.dir.x = -cos(cam.rot.z);
				[self setNeedsDisplay: YES];
				break;
